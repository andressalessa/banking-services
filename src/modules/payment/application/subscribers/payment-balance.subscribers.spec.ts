...
import { OnExpenseScheduled } from './on-expense-scheduled.subscriber';
import { OnExpensePaid } from './on-expense-paid.subscriber';
import { OnExpenseCancelled } from './on-expense-cancelled.subscriber';
import { OnExpenseFailed } from './on-expense-failed.subscriber';
import { OnExpenseRefunded } from './on-expense-refunded.subscriber';
...
describe('Payment balance subscribers', () => {
  ...
  it('should reserve balance when an expense is scheduled', async () => {
    new OnExpenseScheduled(new ReserveAccountBalanceUseCase(accounts));
  ...
  it('should debit reserved balance when an expense is paid', async () => {
    const account = accounts.items[0];
    account.reserveBalance(Money.create(200), 'expense:expense-1');
    account.clearEvents();
    await accounts.save(account);

    new OnExpensePaid(new ConfirmAccountDebitUseCase(accounts));
    const expense = makeExpense();
    expense.approve('approver-person-id');
    expense.markAsPaid();
    await DomainEvents.dispatchEventsForAggregate(expense.id);
  ...
  it('should release reserved balance when a scheduled expense is cancelled', async () => {
    ...
    new OnExpenseCancelled(new ReleaseAccountBalanceUseCase(accounts));
    const expense = makeExpense();
    expense.approve('approver-person-id');
    expense.schedule();
    expense.clearEvents();
  ...
  it('should release reserved balance when an expense fails', async () => {
    ...
    new OnExpenseFailed(new ReleaseAccountBalanceUseCase(accounts));
    const expense = makeExpense();
    expense.approve('approver-person-id');
    expense.schedule();
    expense.clearEvents();
  ...
  it('should credit balance when an expense is refunded', async () => {
    new OnExpenseRefunded(new CreditAccountBalanceUseCase(accounts));
    const expense = makeExpense();
    expense.approve('approver-person-id');
    expense.markAsPaid();
    expense.clearEvents();
